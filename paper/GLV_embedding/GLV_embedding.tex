...
case of a single population experiencing an Allee effect: under the
critical threshold \(\alpha\), the population declines; between
\(\alpha < y < \beta\), it grows; and, finally, it declines if the
density is above \(\beta\). Growth in this model can be represented as a
cubic polynomial:
...
population, \(y^\star = \alpha\) and \(y^\star = \beta\). We want to
show that the equilibrium \(y^\star = \beta\) is asymptotically stable
for any trajectory starting at \(y(0) > \alpha\). First, we rewrite the
system in QP form, and then turn it into a two-dimensional GLV system:
