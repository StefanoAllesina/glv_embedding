...
**LaSalle's invariance principle.** The condition $\dot{V}(y) < 0$ can be often relaxed to $\dot{V} \leq 0$. In such cases, one can apply LaSalle's invariance principle and conclude that any trajectory starting in $\mathcal D$ will converge to the *largest invariant set* included in set of points for which $\dot{V}(y) = 0$. In practice, we can prove stability of the equilibrium whenever we can identify a function $V(y)$ such that $\dot{V} \leq 0$, and the equilibrium is the only trajectory contained in the sub-space of $\mathcal D$ defined by $\dot{V}(y) = 0$.
